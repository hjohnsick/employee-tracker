...
class Query {
  ...
  viewDepartmentChoices(rolePrompts, startApplication) {
    ...
    db.query(sql, (err, rows) => {
      ...
      rolePrompts(departments, startApplication);
    });
  }

  // Display employees role title for adding a new employee
  viewTitleChoices(managers, employeePrompts) {
    const sql = `SELECT title, id FROM role`;
    db.query(sql, (err, rows) => {
      if (err) throw err;
      const titles = rows.map((role) => ({
        name: role.title,
        value: role.id,
      }));
      employeePrompts(managers, titles);
    });
  }
  ...
  viewManagerChoices(employeePrompts) {
    ...
    db.query(sql, (err, rows) => {
      ...
      this.viewTitleChoices(managers, employeePrompts);
    });
  }
