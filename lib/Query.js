...
class Query {
    ...
    // viewEmployeeChoices(updateRolePrompts) {
    ...
    //     db.query(sql, (err, rows) => {
    //         if (err) throw err;
    //         employees = rows.map(employee => ({ name: employee.name, value: employee.id}));
            
    //     });
    //     const roleSql = `SELECT title, id FROM role`;
    //     db.query(roleSql, (err, rows) => {
    //         if (err) throw err;
    //         titles = rows.map(title => ({ name: title.title, value: title.id }));
