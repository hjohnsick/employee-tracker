...
class Query {
    ...
    addEmployeeChoices(employeePrompts) {
        ...
        db.query(sql, (err, rows) => {
            ...
            const filteredTitles = rows.filter((elem, index) => titles.findIndex(obj => obj.value === elem.value) === index);
    ...
    viewUpdateRoleChoices(updateRolePrompts) {
        const sql = `SELECT DISTINCT role.title, role.id, employee.first_name, employee.last_name, employee.id FROM employee
